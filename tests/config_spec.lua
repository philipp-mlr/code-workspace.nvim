...
describe("config.resolve", function()
    ...
    it("returns all defaults when called with no arguments", function()
        ...
        assert.equals("snacks", c.explorer)
    end)
    ...
    it("accepts nvim_tree as the explorer backend", function()
        local c = config.resolve({ explorer = "nvim_tree" })
        assert.equals("nvim_tree", c.explorer)
    end)
